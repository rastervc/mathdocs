...
***Definition.*** (Rings) An ordered triple $(R, +, \cdot)$ is called a *ring* if it satisfies the following properties.
...
***Definition.*** (Trivial rng) An rng $R$ is *trivial* if $xy = 0$ for all $x, y \in R$.
...
***Definition.*** (Subring) Let $R$ be a ring. A subgroup $S$ of $(R, +)$ is a *subring* if $(S, \cdot)$ is a submonoid of $(R, \cdot)$. We define *subrngs* similarly, but we only require that $S$ is closed under multiplication.

***Note.*** A subrng can be a ring, but not a subring if its $1$ does not coincide with the $1$ of the overring.

***Definition.*** (Proper subring) Let $R$ be an ring and $S \subset R$ a subring. We call $S$ a *proper subring* if it is neither $R$ nor the zero ring. These are called the *improper subrings* of $R$.

### The lattice of subrings

***Definition.*** Let $R$ be a ring. We define $\mathcal{S}(R)$ to be its set of subrings.
...
***Definition.*** Suppose $R$ is a ring and $X$ is a subset of $R$. We define the *subring generated by $X$* to be the intersection of all subrings containing $X$, denoted by
<div>$$ \langle X \rangle = \bigcap_{\substack{S \text{ subring} \\ X \subset S}}{S} $$</div>
In other words, $\langle X \rangle$ is the smallest subring of $R$ containing $X$.
...
***Definition.*** (Modules) An abelian group $(M, +)$, along with a ring $R$ and a binary operation $\cdot: R \times M \to M$ is called a *left $R$-module* or a *left module over $R$* if it satisfies the below properties.
...
***Definition.*** (Bimodules) Suppose $R$ and $S$ are two rings and $(M, +)$ is an abelian group. Then $M$ is an *$(R,S)$-bimodule* if it is a left $R$-module, a right $S$-module, and it satisfies the associativity law $ (rx)s = r(xs) $ for every $r \in R$, $x \in M$ and $s \in S$. The $(R, S)$-bimodule structure is commonly emphasized as $_RM_S$.
...
***Example.*** Any ring $R$ can be given the structure a left $R$-module, a right $R$-module, and an $(R,R)$-bimodule by defining scalar multiplication as expected: $r \cdot x = rx$ and $x \cdot r = xr$.
...
***Definition.*** Let $M$ be a left $R$-module. We define $\mathcal{S}(R)$ to be its set of submodules. If $M$ is a bimodule, we let $\mathcal{S}(M)$ denote its set of sub-bimodules.

***Note.*** Let $M$ be an $(R,S)$-bimodule. Then the set of left submodules is $\mathcal{S}(_RM)$ and the set of right submodules is $\mathcal{S}(M_S)$.
...
***Definition.*** Suppose $M$ is a left $R$-module and $X$ is a subset of $M$. We define the *submodule generated by $X$* to be the intersection of all submodules containing $S$, denoted by
<div>$$\langle X \rangle = \bigcap_{\substack{N \text{ submodule} \\ X \subset N}}{N}$$</div>
In other words, $\langle X \rangle$ is the smallest submodule of $M$ containing $X$.

***Note.*** In the case of an $(R, S)$-bimodule, we have $_R\langle X \rangle$, the generated left $R$-submodule, $\langle X \rangle_S$, the generated right $S$-submodule and $\langle X \rangle = _{R\!\!}\langle X \rangle_S$, the generated sub-bimodule.

TODO :: FACTS ABOUT GENERATORS, LINEAR INDEPENDENCE

***Definition.*** A (bi)module with a finite set of generators is called a *finitely generated (bi)module*.

***Definition.*** A (bi)module generated by a single element is called a *cyclic module*.

### Ideals

TODO :: BASIC STUFF ABOUT IDEALS, SEMIRING OF IDEALS, SUMS AND PRODUCTS OF SETS IN RINGS

***Note.*** In the previous section we defined *cylic submodules*. Ideals generated by a single element are more commonly called *principal (left, right) ideals* rather than cyclic.

### Submodule conditions

TODO :: DEFINE ACC AND DCC

***Definition.*** Let $M$ be a left $R$-module. We say $M$ is *Noetherian* if its submodules satisfy the ACC.

***Definition.*** Let $M$ be an $(R, S)$-bimodule. We say $M$ is *left Noetherian* if it is Noetherian as a left module. Similarly, we say $M$ is *right Noetherian* if it is Noetherian as a right module. If $M$ satisfies both properties, then it is *Noetherian*.

***Note.*** A ring $R$ is left/right Noetherian or just Noetherian if it is so as an $(R, R)$-bimodule.

<div class="compact" markdown>

***Theorem.*** Suppose $M$ is a left $R$-module. Then $M$ is Noetherian if and only if every submodule is finitely generated.

*Proof.* First let $M$ be Noetherian and suppose there is a submodule $N$ that is not finitely generated. We will recursively construct a sequence $x_n$ of elements of $N$ such that 
<div>$$\langle x_1, \ldots, x_n \rangle \subsetneq \langle x_1, \ldots, x_{n + 1} \rangle $$</div>
to obtain a contradiction. First choose some $x_1 \in N$. Now, suppose we have chosen the first $n$ elements and are looking for the next one. Since $N$ is not finitely generated, there must be some $x_{n + 1}$ in $N$ that is not in the submodule generated by the already found values.

Now let $M$ be a left $R$-module such that every submodule is finitely generated and suppose there is an infinite strictly increasing sequence of submodules 
<div>$$N_1 \subsetneq N_2 \subsetneq \ldots$$</div>
Their union, a submodule $N$, must be finitely generated by some $x_1, \ldots x_n$. Then there exist a positive integer $k$ such that $N_k$ contains all $x_i$, and hence $N_k = N$. This shows that the sequence is not strictly increasing.
<span style="float: right;">$\square$</span>

</div>

***Definition.*** A ring $R$ is a *principal left (right) ideal ring* if every left (right) ideal in $R$ is principal. If it satisfies both properties, it is called a *principal ideal ring* or sometimes just a *principal ring*.

***Definition.*** Let $M$ be a left $R$-module. We say $M$ is *Artinian* if its submodules satisfy the DCC.

***Definition.*** Let $M$ be an $(R, S)$-bimodule. We say $M$ is *left Artinian* if it is Artinian as a left module. Similarly, we say $M$ is *right Artinian* if it is Artinian as a right module. If $M$ satisfies both properties, then it is *Artinian*.

***Note.*** A ring $R$ is left/right Artinian or just Noetherian if it is so as an $(R, R)$-bimodule.
